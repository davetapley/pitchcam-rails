...
RUN apt-get update && apt-get install -qq -y --no-install-recommends \
      build-essential nodejs libpq-dev libopencv-dev
...
ENV INSTALL_PATH /pitchcam_rails
# The name of the application is pitchcam_rails and while there
...
# having to CD into the /pitchcam_rails folder every time would be
# lame, so instead we can set the WORKDIR to be /pitchcam_rails.
#
...
# pitchcam_rails/ path inside of the Docker image.
#
# It copies it to /pitchcam_rails because of the WORKDIR being set.
#
...
# /pitchcam_rails folder inside of the Docker image.
#
